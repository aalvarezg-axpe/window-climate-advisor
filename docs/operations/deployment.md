...
- Candidate: GitHub prerelease `v0.2.0rc2` from `release/0.2.0`
...
1. In the existing HACS custom repository, select and download `v0.2.0rc2`.
   Enable prerelease tracking if HACS hides beta versions.
2. Confirm HACS reports exactly `v0.2.0rc2` before continuing.
...
2. While `v0.2.0rc2` is loaded, remove recipient subentries through the config
