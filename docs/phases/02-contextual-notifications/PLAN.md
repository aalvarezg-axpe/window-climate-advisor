...
| P02-T04 | Validate notification behaviour in Home Assistant before release. Accept setup/unload/reload/restart, unavailable person/notify target and send-failure recovery, present/away/mixed/arrival scenarios, Recorder-safe privacy evidence, exact notification counts, no duplicates, no owned persistent backlog, and zero physical services/actions. | en curso | root manager | integration tests, manifest/changelog/version, deployment/status/runbook docs, this plan, Git/HACS release state and Home Assistant external verification | `v0.2.0b1` proved installation/lifecycle with zero recipients but its explicit target contract was superseded by owner decision. Complete this gate against P02-T05's `v0.2.0b2`; retain live-verified `v0.1.0b5` as integration fallback and `v4.17_pre` as operational rollback. |
| P02-T05 | Replace the redundant person-to-notify selection with native person-to-Mobile-App-device discovery. Accept person-only create/reconfigure, schema-v4 revision-1→2 recipient migration with downgrade compatibility, all-and-only home linked-device delivery, multiple devices per person, duplicate-call suppression, missing/disabled/unavailable registry paths, redacted diagnostics, and a production-boundary test using registry relationships rather than names. Publish/install `v0.2.0b2`, register all four current persons through supported subentry flows, and verify all five associated mobile targets without sending a synthetic message or creating physical actions. | en curso | root manager retained: public schema/migration and prior executor permission mismatch | `AGENTS.md`, `docs/GOAL.md`, `const.py`, `__init__.py`, `config_flow.py`, `application/notifications.py`, `adapters/notifications.py`, translations, focused config/migration/application/delivery/init tests, ADR/README/changelog/version/deployment status/runbook/this plan, Git/HACS release state and supported Home Assistant config flows | Owner corrected the beta contract after a redacted live registry audit proved 4/4 persons have linked Mobile App trackers: three resolve to one target and one to two. No live recipient exists yet, so the correction precedes real recipient configuration; the published b1 compatibility path remains tested. |
...
| 2026-08-30 | P02-T05 | Owner rejected the redundant explicit notification target and authorized registering every person and associated device. A redacted read-only audit found four configured persons, five linked Mobile App trackers with five sibling notification entities, three one-device persons and one two-device person; no person entity itself owns a registry device, confirming the native relationship is indirect but complete. Corrected the living AGENTS/GOAL/plan contract and froze a schema-v4 revision-2 migration plus b2 deployment before implementation. | No Home Assistant state or repository code changed during discovery. The minimum design stores only persons, joins registry siblings without names, filters each tracker by its own `home` state, calls only fixed `notify.send_message`, and uses supported subentry flows for registration. The major schema deliberately stays v4 so `v0.1.0b5` remains a loadable downgrade; P02-T04 remains open and b1 is superseded rather than configured. |
