...
| P01-T10 | Deploy reversibly beside operational v4.17_pre and run four consecutive calendar days of owner-approved shadow observation. Accept exact route/target, backup, rollback, configuration validation, one entry, expected entities without duplicates, clean reload/restart, no owned notifications/actions, and an approved comparison report. | en curso | root manager | `AGENTS.md`, `hacs.json`, `README.md`, `.env.example`, `CHANGELOG.md`, integration manifest/brand, `tests/integration/test_manifest.py`, `docs/GOAL.md`, `docs/operations/development.md`, `docs/operations/deployment.md`, `docs/status/phase01-shadow.md`, this plan, local/remote Git state, Home Assistant external state | Public remote, `main` default, feature/release branches, and HACS prerelease `v0.1.0b1` are published and verified at candidate commit `dc83bf5`. No mounted-path or SSH variables are needed. Owner froze four days on 2026-08-25 because the interval should contain rain, heat, and sun. Next mutation is the named Home Assistant backup followed by HACS installation. |
