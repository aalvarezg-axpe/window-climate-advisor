...
| P01-T10 | Deploy reversibly beside operational v4.17_pre and run four consecutive calendar days of owner-approved shadow observation. Accept exact route/target, backup, rollback, configuration validation, one entry, expected entities without duplicates, clean reload/restart, no owned notifications/actions, and an approved comparison report. | en curso | root manager | `AGENTS.md`, `hacs.json`, `README.md`, `.env.example`, `CHANGELOG.md`, integration manifest/brand, `tests/integration/test_manifest.py`, `docs/GOAL.md`, `docs/operations/development.md`, `docs/operations/deployment.md`, `docs/status/phase01-shadow.md`, this plan, local/remote Git state, Home Assistant external state | The named full baseline backup completed and v4.17_pre remains available. HACS bootstrap uses `release/0.1.0` temporarily as the GitHub default without making `main` releasable. Installed `v0.1.0b1` exposed a live-only form serialization defect before entry creation; P01-T12 owns the bounded correction and immutable replacement candidate `v0.1.0b2`. |
...
| P01-T12 | Correct the Home Assistant 2026.8 live config-flow serialization failure found during P01-T10. Accept a serializable native name validator that still rejects empty/whitespace-only names, an HTTP-equivalent schema serialization regression, canonical verification, immutable `v0.1.0b1`, and a new numbered beta candidate. | en curso | root manager | `config_flow.py`, focused config-flow tests, manifest/changelog/version metadata, deployment/status docs, this plan, release/tag/prerelease/HACS external state | Live Core raised `ValueError: Unable to convert schema` for the plain `_non_empty_name` callable before any config entry was created. Keep the correction local to schema validation and publish `v0.1.0b2`; no options default, entity, service, notification, helper, or actuator may be added. |
