...
| P01-T10 | Deploy reversibly beside operational v4.17_pre and run the owner-agreed shadow period. Accept exact route/target, backup, rollback, configuration validation, one entry, expected entities without duplicates, clean reload/restart, no owned notifications/actions, and an approved comparison report. | en curso | root manager | `AGENTS.md`, `hacs.json`, `README.md`, `.env.example`, `CHANGELOG.md`, integration manifest/brand, `tests/integration/test_manifest.py`, `docs/GOAL.md`, `docs/operations/development.md`, `docs/operations/deployment.md`, `docs/status/phase01-shadow.md`, this plan, local/remote Git state, Home Assistant external state | P01-T09/P01-T11 are terminated. Owner authorized public `aalvarezg-axpe/window-climate-advisor` for Git/HACS on 2026-08-25; no mounted-path or SSH variables are needed. Preserve `main` as the accepted-release branch, publish the current feature branch for review, and freeze `v0.1.0b1` as a HACS prerelease from `release/0.1.0` before installing or mutating Home Assistant. |
