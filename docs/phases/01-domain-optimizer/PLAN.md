...
| P01-T10 | Deploy reversibly beside operational v4.17_pre and run the owner-agreed shadow period. Accept exact route/target, backup, rollback, configuration validation, one entry, expected entities without duplicates, clean reload/restart, no owned notifications/actions, and an approved comparison report. | bloqueada | root manager | `.env.example` route names if needed, `docs/operations/deployment.md`, `docs/status/phase01-shadow.md`, Home Assistant external state | P01-T09/P01-T11 are terminated. The owner selected Git/HACS on 2026-08-25, so no mounted-path or SSH variables are needed. Current HACS requires a public GitHub repository; this local repository has no remote and `docs/GOAL.md` still excludes public distribution. Freeze the authorized public repository URL, default branch/release, and rollback commands before implementation or external mutation. |
