...
| P01-T07 | Add cost/benefit hysteresis and stable transitions. Accept no churn from small changes, the 15-minute blind policy, at most one grouped notification candidate, and notification delivery absent in shadow mode. | terminada | root manager while the persistent executor remains suspended | `domain/state_machine.py`, `application/__init__.py`, `application/state.py`, minimal optimizer result/test additions, state-machine/application tests, behaviour-matrix dispositions | Focused 24/24 and canonical 91/91 passed at 100%. P01-T08 triage confirmed 33 candidates map one-to-one to 33 stable action transitions at segment boundaries/first confirmation, with no intra-segment churn; the rejected `33 ≤ 28` assertion compared legitimate new thermal decisions to legacy raw transitions rather than false alerts. No production correction was required. |
